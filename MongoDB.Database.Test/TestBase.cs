...
namespace MongoDB.Database.Test
{
    ...
    public abstract class TestBase
    {
        ...
        public void Setup()
        {
            ...
            _reader = new MongoDB.Database.Reader<TestEntry>(MONGO_CONNECTION_STRING, MONGO_DATABASE_1_NAME);
            ...
            _writer = new MongoDB.Database.Writer<TestEntry>(MONGO_CONNECTION_STRING, MONGO_DATABASE_1_NAME);
            _writer.AsyncWriteCompleted += new WriteCompletedEvent(_writer_AsyncWriteCompleted);
            _updater = new MongoDB.Database.Updater<TestEntry>(MONGO_CONNECTION_STRING, MONGO_DATABASE_1_NAME);
        ...
        protected IDatabaseReader<TestEntry>  _reader;
        protected IDatabaseWriter<TestEntry>  _writer;
        protected IDatabaseUpdater<TestEntry> _updater;
        ...
        protected const string MONGO_CONNECTION_STRING_BAD = "mongodb://ss";
        ...
        protected void AddMongoEntryAsync(string message = "Hello World", string collectionName = MONGO_COLLECTION_1_NAME)
        {
            ...
            _writer.WriteAsync(collectionName, mongoEntry);
            _writerAutoResetEvent.WaitOne();// wait two seconds for the autoresetevent to be signaled in _mongoWriterAsync_WriteCompletedEvent
        }
        ...
        void _reader_AsyncDistinctCompleted(IEnumerable<BsonValue> e, Exception ex)
        {
            _asyncException = ex;
